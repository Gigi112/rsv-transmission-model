...
loglik_function <- function(parameters,dat) {   # takes as inputs the parameter values and dataset
  ...
  phi <<- phi
   ...
   LL <- sum(dpois(x = dat$rsv, lambda = 1.5 * output$`3`[output$time %in% dat$time], log = TRUE)) 
   return(LL)
   }

optim(par = c(7.9,.38,0.5),           # starting values for beta and gamma - you should get the same result no matter 
      ...
      control = list(fnscale=-1))  # tells optim() to look for the maximum number 
