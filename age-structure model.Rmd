...
my_model <- function(times,x,pars){
  ...
  lamda=(1+b1*cos(2*pi*(times-phi*52.18)/52.18))*beta%*%(x[(2*al+1):(3*al)]+rho1*x[(4*al+1):(5*al)]+rho2*x[(6*al+1):(7*al)]+rho2*x[(8*al+1):(9*al)])/sum(x)
  
	for (a in 1:nage) {
	  
	  dx[a] <- log(B[times,a]+1)/52*sum(x)-(omega+mu[a])*M[a]
		...
		dx[a+4*nage] <- sigma1*lambda[a]*S1[a]- gamma1*I2[a]-mu[a]*I2[a]
		dx[a+5*nage] <- gamma1*I2[a]- sigma2*lambda[a]*S2[a]-mu[a]*S2[a]
		dx[a+6*nage] <- sigma2*lambda[a]*S2[a]-gamma2*I3[a]-mu[a]*I3[a]
		dx[a+7*nage] <- gamma2*I3[a]-sigma3*lambda[a]*S3[a]+gamma3*I4[a]-mu[a]*S3[a]
		dx[a+8*nage] <- sigma3*lambda[a]*S3[a]-gamma3*I4[a]-mu[a]*I4[a]
...
parms<-list(B=B,omega=omega,mu=mu,beta=beta,b1=b1,phi=phi,rho1=rho1,rho2=rho2, gamma1=gamma1,sigma1=sigma1,gamma2=gamma2,sigma2=sigma2,gamma3=gamma3,sigma3=sigma3)
