...
for (t in 1:length(St))
    {lambda[t] <- lambda[a] <- beta[a,]%*%tau
		beta <- (1+b1*cos(2*pi*(t-phi*52.18)/52.18))*beta0}
...
{Pop=Pop+x[al*(i-1)+1:al*i]}

#Other outcomes of interest
for (i in 1:al){
  
    C[i] <- delta1[i]*x[,(al+i)]*lambda[i]+delta2[i]*rr1*x[,(3*al+i)]*lambda[i]+delta3[i]*rr2*x[,(5*al+i)]*lambda[i]+delta3[i]*rr3*x[,(7*al+i)]*lambda[i]
    
    H[i]=hosp1[i]*x[,(al+i)]*lambda[i]+hosp2[i]*rr1*x[,(3*al+i)]*lambda[i]+hosp3[i]*rr2*x[,(5*al+i)]*lambda[i]+hosp4[i]*rr3*x[,(7*al+i)]*lambda[i]
    
    Incid[i]=x[,(al+i)]*lambda[i]+rr1*x[,(3*al+i)]*lambda[i]+rr2*x[,(5*al+i)]*lambda[i]+rr3*x[,(7*al+i)]*lambda[i]
                                                                                                                            
    Prev[i]=x[,(2*al+i)]+x[,(4*al+i)]+x[,(6*al+i)]+x[,(8*al+i)]}
...
x <- matrix(0,nrow=3364,ncol = 189)
popagedist=colMeans(x[,1:al]+x[,(al+1):(2*al)]+x[,(2*al+1):(3*al)]+x[,(3*al+1):(4*al)]+x[,(4*al+1):(5*al)]+x[,(5*al+1):(6*al)]+x[,(6*al+1):(7*al)]+x[,(7*al+1):(8*al)]+x[,(8*al+1):(9*al)])/(mean(rowSums(x)))
