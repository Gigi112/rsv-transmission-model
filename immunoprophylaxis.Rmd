...
Sv0[(al+1):(2*al)]=c((N[1:6]*v*eff), rep(0,al-6))#immunoprophylaxis immunity
Sv0[(2*al+1):(3*al)]=c(rep(0,6),N[7:al]-rep(1,al-6))#Susceptible_0
Sv0[(3*al+1):(4*al)]=c(rep(0,6),rep(1,al-6))#Infectious_1 (primary) 
...
passive_immune <- function(times,x,pars){
  ...
  lambda=(1+b1*cos(2*pi*(times-phi*52.18)/52.18))%*%t(x[(3*al+1):(4*al)]+rho1*x[(5*al+1):(6*al)]+rho2*x[(7*al+1):(8*al)]+rho2*x[(9*al+1):(10*al)])%*%beta/sum(x)
	...
	for (a in 1:nage) {
	  ...
	  dx[a] <- log((1-v*eff)*B[times,a]+1)/52*sum(x)-(omega+(u[a]+um))*M[a] #dM/dt
		dx[a+nage] <- log(v*eff*B[times,a]+1)/52*sum(x)-(omega_v+(u[a]+um))*Mv[a] #dMv/dt
