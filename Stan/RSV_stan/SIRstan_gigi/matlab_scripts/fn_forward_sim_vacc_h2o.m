function [out, pop17] = fn_forward_sim_vacc_h2o(params, agepar, estpar, vacpar, population, tspan, site)
...
betap = betanew*ones(al, length(1:(1+52*30))).*repmat(1:-0.5/length(1:(52*30)):0.5, al, 1);
betap(agepar.under5==1,:) = mult1new*betap(agepar.under5==1,:) ;
betap(agepar.under5==2,:) = mult2new*betap(agepar.under5==2,:) ;

[t,y] = ode45(@(t, pop) fn_SIR_vaccine_vnv_strata_h2o(t, pop, betap, mub, u, mu_real, omega, omega_v, r, rC, alpha, delta, theta, theta2, epsilon, v1, veff, massvacc, al, population), 1:(1+52*30), pop17, options);
...
% PEOPLE EFFECTIVELY VACCINATED at any point in time
         out.V1 = y(obs_time, 6*al+1:7*al) ; %V1
         out.V2 = y(obs_time, 7*al+1:8*al) ; %V2
