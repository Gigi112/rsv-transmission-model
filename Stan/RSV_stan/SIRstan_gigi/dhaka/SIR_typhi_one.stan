functions {
       ...
       real[] make_params(real beta, 
                real logm1,
                real logm2,
                real r,
                int agegroups) {
                real params[agegroups+1];
                params[1] <- exp(-logm1)*beta;
                params[2] <- (1-exp(-logm2))*beta;
       ...
       real[] sir(real t,
           ...
           int[] x_i) { // x_i will be the number of age groups, whether I2 has zero chance of becoming a chronic carrier, and whether I2 contributes the same as I1 or as C.
            ...
            for (j in 1:1) {
                ...
                dydt[j+x_i[1]*7] <- theta[j]*(sum(y[(x_i[1]+1):(2*x_i[1])])+(theta[x_i[1]+1]*x_i[3]+(1-x_i[3]))*sum(y[(4*x_i[1]+1):(5*x_i[1])])+theta[x_i[1]+1]*sum(y[(5*x_i[1]+1):(6*x_i[1])]))/x_r[1] - y[j+x_i[1]*7]; // LAMBDA: beta*[(first-time infections) + r*(indicator for 1 or fraction)*(second-time infections) + r*chronics]/population
            }
...
transformed parameters {  
       ...
       params <- make_params(beta, logm1, logm2, r, agegroups);
...
model {
    ...
    // Disease process parameters
        ...
        r ~ beta(1.9173, 57.4160); // Same parameterization as in matlab. 2x the parameters with Ginny.
    
    // Run ODE
       yhat <- integrate_ode(sir, y0[1:agegroups*8], t0, tspan, params, x_r, x_i);
    ...
    // Likelihood
    for (i in 1:agegroups)
        cases[i] ~ poisson((yhat[ts, (i+agegroups*6)] - yhat[ts-52, (i+agegroups*6)])*participation[i]*blood[i]*prop); 
    if (dhaka==1){
