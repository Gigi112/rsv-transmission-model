...
yinit <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/CA/yinit.rds') #intial population
birth <-  readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/CA/BrCA.rds')
contactUSAinfant <- readRDS( 'C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/contactmatrix_nodiff.rds') # contact matrix
ca_agedist <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/CA/ca_agedist.rds')
rsv_ca <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/CA/rsv_ca.rds')
```
...
ca_times <- seq(start_time, tmax, by = 1) # gives a sequence from start to end
											 # in increments of 1
```
...
```{r}
results_test <- ode(y=yinit.vector, t=ca_times,  
               func=Novac_dynamics, 
            parms=parmstest)
...
rowSums(results_test[,-1])[12*23]# should equals to about 35.24 million
```

## Extract the relevant results to see if the epidemic reach equilibrium
```{r}
#used a 23-YEARS burn in period
burnN <- 23*12
...
## fit the model to ca inpatient data in order to estimate inital value of parameters
```{r}
...
fitmodel <-  function(parameters,dat) {   # takes the parameter values and dataset as inputs
  ...
  results <- ode(y=yinit.vector, method = "ode45", t=ca_times,  
               func=Novac_dynamics, 
  ...
  H <- report_ratio*rowSums(H1)
   ...
   LLmulti <- dmultinom(x= age_dist[1:8],prob = ca_agedist[1:8],log = T) # fit to age
...
fitLL <- optim(par = c( 1.980926 ,-1.451419, -1.222989,  3.193689, -0.17
      ...
      dat = rsv_ca$rsv,         # the dataset to fit to ("dat" argument is passed to the function specified in fn)
...
```{r,eval=F}
  ...
  report_ratio <- exp(parameters[5])
  hosp3 <- exp(parameters[6])

  output <- ode(y=yinit.vector, t=ca_times,  
               func=Novac_dynamics, 
   ...
   H1=matrix(0,nrow=t0,ncol=al)#Number of hospitalizations by age
      for (i in 1:al){
     H1[,i]=report_ratio*(hosp1[i]*S0[,i]*lambda1[,i]+hosp2[i]*sigma1*S1[,i]*lambda1[,i]+hosp3[i]*sigma2*S2[,i]*lambda1[,i]+hosp3*hosp2[i]*sigma3*S3[,i]*lambda1[,i])}
  H <- rowSums(H1)
  age_dist <- colSums(H1)
   H <- data.frame(H)
   H$date <- rsv_ca$amonthdate
```

## Real Data vs Model Fit
### time series
```{r}
plot(rsv_ca$amonthdate,rsv_ca$rsv,xlab = "date",ylab="RSV cases",type="l")
...
# create a dataset
agedist <- rbind(age_dist,ca_agedist)
...
```{r}
rsv_ca$months <- rep(c(7:12,1:6),8)
library(tidyverse)
hosp_cases <- rsv_ca %>%
  group_by(months) %>%
...
stan_initial_para <- function()list(beta=7.249465,b1=0.23426488,phi=1.42886322,omega=0.04102234,hosp_ratio=0.03116693)
saveRDS(stan_initial_y,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/stan_initial_y_ca.rds")
saveRDS(stan_initial_para,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/stan_initial_para_ca.rds")
data_msis_ca <- list(n_months=12,y0=stan_initial_y,t0=0,ts=c(1:12),N=sum(stan_initial_y),birthrate=c(0.015,rep(0,12)),um=um,rho1=rho1,rho2=rho2,gamma1=gamma1,gamma2=gamma2,gamma3=gamma3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,hosp1=hosp1,hosp2=hosp2,hosp3=hosp3,c2=as.vector(contactUSAinfant),q=1,agegroups=13,hosp_age=as.vector(ca_agedist)[1:8],u=1/WidthAgeClassMonth,hosp_cases=hosp_cases$rsv)
saveRDS(data_msis_ca,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/data_msis_ca.rds")

data_msis_ca_9yrs <- list(n_months=nrow(rsv_ca),y0=stan_initial_y,t0=0,ts=c(1:nrow(rsv_ca)),N=sum(stan_initial_y),birthrate=c(0.015,rep(0,12)),um=um,rho1=rho1,rho2=rho2,gamma1=gamma1,gamma2=gamma2,gamma3=gamma3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,hosp1=hosp1,hosp2=hosp2,hosp3=hosp3,c2=as.vector(contactUSAinfant),q=1,agegroups=13,hosp_age=as.vector(ca_agedist),u=1/WidthAgeClassMonth,hosp_cases=rsv_ca$rsv)
saveRDS(data_msis_ca,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/data_msis_ca_9yrs.rds")
```
