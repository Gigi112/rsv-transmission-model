...
yinit <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/NY/yinit.rds') #intial population
birth <-  readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/NY/BrNY.rds')
contactUSAinfant <- readRDS( 'C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/contactmatrix.rds') # contact matrix
ny_agedist <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/NY/ny_agedist.rds')
rsv_ny <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/NY/rsv_ny.rds')
```
...
ny_times <- seq(start_time, tmax, by = 1) # gives a sequence from start to end
											 # in increments of 1
```
...
```{r}
results_test <- ode(y=yinit.vector, t=ny_times,  
               func=Novac_dynamics, 
            parms=parmstest)
...
## fit the model to NY inpatient data in order to estimate inital value of parameters
```{r}
...
fitmodel <-  function(parameters,dat) {   # takes the parameter values and dataset as inputs
  ...
  results <- ode(y=yinit.vector, t=ny_times,  
               func=Novac_dynamics, 
  ...
  H <- report_ratio*rowSums(H1)
   ...
   LLmulti <- dmultinom(x= age_dist[1:8],prob = ny_agedist[1:8],log = T) # fit to age distribution
...
fitLL <- optim(par = c(2.056407e+00, -1.840634e+00, -1.292155e+00,  2.828397e+00, -1.180958e-05),           # 
      ...
      dat = rsv_ny$H,         # the dataset to fit to ("dat" argument is passed to the function specified in fn)
...
```{r,eval=F}
  ...
  output <- ode(y=yinit.vector, t=ny_times,  
               func=Novac_dynamics, 
...
### time series
```{r}
plot(rsv_ny$date,rsv_ny$H,xlab = "date",ylab="RSV cases",type="l")
...
# create a dataset
agedist <- rbind(age_dist/sum(age_dist),ny_agedist/sum(ny_agedist))
...
```{r}
rsv_ny$months <- rep(c(7:12,1:6),9)
library(tidyverse)
hosp_cases <- rsv_ny %>%
  group_by(months) %>%
  summarise(rsv=as.integer(mean(H)))
...
data_msis_ny_9yrs <- list(n_months=nrow(rsv_ny),y0=stan_initial_y,t0=0,ts=c(1:nrow(rsv_ny)),N=sum(stan_initial_y),birthrate=c(0.013,rep(0,12)),um=um,rho1=rho1,rho2=rho2,gamma1=gamma1,gamma2=gamma2,gamma3=gamma3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,hosp1=hosp1,hosp2=hosp2,c2=contactUSAinfant,q=1,agegroups=13,hosp_age=age_dist,u=1/WidthAgeClassMonth,hosp_cases=rsv_ny$rsv)
saveRDS(data_msis_ny_9yrs,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/data_msis_ny_9yrs.rds")

data_msis_ny_annual <- list(n_months=12,y0=stan_initial_y,t0=0,ts=1:12,N=sum(stan_initial_y),birthrate=c(0.013,rep(0,12)),um=um,rho1=rho1,rho2=rho2,gamma1=gamma1,gamma2=gamma2,gamma3=gamma3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,hosp1=hosp1,hosp2=hosp2,c2=contactUSAinfant,q=1,agegroups=13,hosp_age=age_dist,u=1/WidthAgeClassMonth,hosp_cases=hosp_cases$rsv)
saveRDS(data_msis_ny_annual,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/data_msis_ny_annual.rds")
```
