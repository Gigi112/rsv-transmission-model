...
yinit <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/WA/yinit.rds') #intial population
birth <-  readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/WA/BrWA.rds')
...
wa_agedist <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/WA/wa_agedist.rds')
rsv_wa <- readRDS('C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/WA/rsv_wa.rds')
```
...
wa_times <- seq(start_time, tmax, by = 1) # gives a sequence from start to end
											 # in increments of 1
```
...
```{r}
results_test <- ode(y=yinit.vector, t=wa_times,  
               func=Novac_dynamics, 
            parms=parmstest)
...
rowSums(results_test[,-1])[12*25]# should equals to about 6.259 million
```
...
## fit the model to wa inpatient data in order to estimate inital value of parameters
```{r}
...
fitmodel <-  function(parameters,dat) {   # takes the parameter values and dataset as inputs
  ...
  H <- report_ratio*rowSums(H1)
   ...
   LLmulti <- dmultinom(x= age_dist[1:8],prob = wa_agedist[1:8],log = T) # fit to age distribution
...
fitLL <- optim(par = c( 2.2162556, -1.6255363, -1.1953393,  3.9579884 ,-0.2657404),           # 
      ...
      dat = rsv_wa$rsv,         # the dataset to fit to ("dat" argument is passed to the function specified in fn)
...
```{r,eval=F}
  ...
  age_dist <- colSums(H1)
   H <- data.frame(H)
   H$date <- rsv_wa$amonthdate
```

## Real Data vs Model Fit
### time series
```{r}
plot(rsv_wa$amonthdate,rsv_wa$rsv,xlab = "date",ylab="RSV cases",type="l")
...
names(age_dist[1:8]) <-  agenames[1:8]

# create a dataset
agedist <- rbind(age_dist[1:8]/sum(age_dist),wa_agedist[1:8]/sum(wa_agedist))
agedist <- data.frame(matrix(agedist,ncol=2,nrow=8,byrow = T))
colnames(agedist) <- c("model","data")
agedist$age <- c(1,2,3,4,5,6,7,8)
...
ggplot(data_long, aes(fill=group, y=proportion, x=age)) + 
  ...
  scale_x_discrete(name ="AGE", 
                    limits=names(age_dist)[1:8])+
  theme_classic()
```

```{r}
rsv_wa$months <- rep(c(7:12,1:6),9)
library(tidyverse)
hosp_cases <- rsv_wa %>%
  group_by(months) %>%
...
stan_initial_para <- list(beta=baseline.txn.rate,b1=b1,phi=phi,omega=1/(DurationMatImmunityDays/30.44),hosp_ratio=report_ratio)
saveRDS(stan_initial_y,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/stan_initial_y_wa.rds")
saveRDS(stan_initial_para,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/stan_initial_para_wa.rds")
data_msis_wa_9yrs <- list(n_months=nrow(rsv_wa),y0=stan_initial_y,t0=0,ts=c(1:nrow(rsv_wa)),N=sum(stan_initial_y),birthrate=c(0.014,rep(0,12)),um=um,rho1=rho1,rho2=rho2,gamma1=gamma1,gamma2=gamma2,gamma3=gamma3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,hosp1=hosp1,hosp2=hosp2,hosp3=hosp3,c2=as.vector(contactUSAinfant),q=1,agegroups=13,hosp_age=wa_agedist,u=1/WidthAgeClassMonth,hosp_cases=rsv_wa$rsv)
saveRDS(data_msis_wa_9yrs,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/data_msis_wa_9yrs.rds")

data_msis_wa_annual <- list(n_months=12,y0=stan_initial_y,t0=0,ts=1:12,N=sum(stan_initial_y),birthrate=c(0.014,rep(0,12)),um=um,rho1=rho1,rho2=rho2,gamma1=gamma1,gamma2=gamma2,gamma3=gamma3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,hosp1=hosp1,hosp2=hosp2,hosp3=hosp3,c2=as.vector(contactUSAinfant),q=1,agegroups=13,hosp_age=wa_agedist,u=1/WidthAgeClassMonth,hosp_cases=hosp_cases$rsv)
saveRDS(data_msis_wa_annual,"C:/Users/70914/Box/aim3/RSVtransmissionmodel/data_and_parms/staninput/data_msis_wa_annual.rds")
```
